...
IO_REG_TYPE directRead(IO_REG_TYPE pin)
{
    ...
    // P4's GPIO struct uses typed registers (union with .val), unlike plain
    // ESP32's raw uint32_t fields — and it has up to 55 pins (GPIO0-54), so
    // GPIO49 needs the in1 (GPIO32-54) register, not just in (GPIO0-31).
    if ( pin < 32 )
        return (GPIO.in.val >> pin) & 0x1;
    else
...
#elif CONFIG_IDF_TARGET_ESP32P4
    // No "pins above 33 are input-only" restriction here — that's a classic
    // ESP32 limitation, not applicable to P4's GPIO layout (e.g. G49).
    if ( digitalPinIsValid(pin) )
    {
        if ( pin < 32 )
            GPIO.enable_w1ts.val = ((uint32_t)1 << pin);
